...
export const getAllProducts = asyncHandler(async (req, res) => {
  ...
  // Price sort can't run in Prisma (price lives on variants, not Product),
  // so sort the formatted page by the effective (lowest) price here.
  if (isPriceSort) {
    const priceOf = (p) => {
      const candidates = [p.basePrice, p.regularPrice].map((v) => parseFloat(v)).filter((n) => !isNaN(n));
      return candidates.length ? Math.min(...candidates) : Number.MAX_SAFE_INTEGER;
    };
    formattedProducts.sort((a, b) =>
      order === "asc" ? priceOf(a) - priceOf(b) : priceOf(b) - priceOf(a)
    );
  }
...
export const getProductBySlug = asyncHandler(async (req, res) => {
  ...
  const formattedProduct = {
    ...
    ingredients: (product.ingredients || []).map((pi) => ({
      ...
      image: pi.ingredient.image ? getFileUrl(pi.ingredient.image) : null,
