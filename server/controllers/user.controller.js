...
export const registerUser = asyncHandler(async (req, res, next) => {
  ...
  try {
    await sendEmail({
      email,
      subject: "Your OTP for Email Verification - MWP SUPPLEMENTS",
...
export const forgotPassword = asyncHandler(async (req, res, next) => {
  ...
  try {
    await sendEmail({
      email,
      subject: "Reset Your Password - MWP SUPPLEMENTS",
...
export const cancelOrder = asyncHandler(async (req, res, next) => {
  ...
  try {
    ...
    if (user && user.email) {
      await sendEmail({
        email: user.email,
        subject: `Order #${order.orderNumber} Cancelled - ${process.env.STORE_NAME || "MWP Supplements"}`,
...
export const requestAccountDeletion = asyncHandler(async (req, res, next) => {
  ...
  try {
    await sendEmail({
      email: user.email,
      subject: "Confirm Account Deletion - MWP SUPPLEMENTS",
