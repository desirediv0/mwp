...
export function CompareTray() {
  ...
  return (
    <AnimatePresence>
      ...
      >
        <div className="pointer-events-auto max-w-3xl mx-auto rounded-2xl bg-gradient-to-br from-[#141416] to-[#0c0c0e] border border-white/10 shadow-[0_24px_70px_-15px_rgba(0,0,0,0.8)] overflow-hidden">
          {/* top accent */}
          ...
          <div className="flex items-center gap-3 sm:gap-4 px-3 sm:px-4 py-3">
            ...
            <div className="flex-1 flex items-center gap-2 overflow-x-auto scrollbar-hide py-0.5">
              {items.map((p) => (
                <Link
                  ...
                  className="group relative shrink-0 w-11 h-11 sm:w-12 sm:h-12 rounded-xl overflow-hidden bg-white/5 border border-white/10 hover:border-red-500/50 transition-colors"
                  title={p.name}
                >
              ...
              {[...Array(Math.max(0, max - count))].map((_, i) => (
                <div
                  key={`empty-${i}`}
                  className="shrink-0 w-11 h-11 sm:w-12 sm:h-12 rounded-xl border border-dashed border-white/12"
                />
              ))}
            </div>
            ...
            <div className="flex items-center gap-2 shrink-0">
              <button
                ...
                className="hidden sm:inline-flex items-center px-3 py-2 rounded-lg text-[11px] uppercase tracking-wider font-bold text-neutral-400 hover:text-white hover:bg-white/5 transition-colors"
              >
                Clear
              </button>
              <Link
                href="/compare"
                className={`inline-flex items-center gap-2 px-4 sm:px-6 py-2.5 rounded-xl text-[11px] sm:text-[12px] uppercase tracking-wider font-extrabold transition-all ${
                  count < 2
